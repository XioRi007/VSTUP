...
export const AddApplication = ()=>{
    ...
    const loadScores = useCallback( async (cleanupFunction)=>{
        try{
            const {Zno} =await request(`/api/applicant/get/zno/${userId}`);      
            if (!Zno.length)
                throw new Error('Ошибка загрузки балів');                
            if(!cleanupFunction) setScores(Zno);
    ...
    const submitHandler = async (e)=>{
        try{
            ...
            const res = await request(`/api/applications/add`, 'PUT', {...doc});            
            if(!res.ok){
    ...
    return (<div className="container" style={{minHeight:'650px'}}>
        ...
        <form className="g-3 col-4 justify-content-center mx-auto" onSubmit={submitHandler}>   
            {( faculties !== undefined) && <div className='row'>
                ...
                <select className="col-5 form-select mb-3" id='Faculty' name='Faculty'  onChange={changeHandler} value={form.Faculty} required>               
            ...
            {(specialties !== undefined) && <div className='row'>
                ...
                <select className="col-5 form-select mb-3" id='Specialty' name='Specialty'  onChange={changeHandler} value={form.Specialty} required>
                    <option value="" selected disabled hidden>Вибрати </option>
                    {specialties.map((i, idx) => {return (<option key={idx} value={i._id} >{i.SpecialtyCode} : {i.Name}</option>)})}                   
                </select>
            </div>}
            <div className='row'>
                <label htmlFor="scores ">Оберіть ЗНО</label>
                <select className="form-select" multiple={true} aria-label="multiple select example" id='scores' name='Scores' onChange={handleChangeMultiple} value={form.Scores} required>
                    {
